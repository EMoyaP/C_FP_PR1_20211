...
void getCoworkingCenterObject(const char *str, tCoworkingCenter *coworkingCenter) {
	...
	int auxId, auxType, auxMeetingRooms, auxKitchen, auxAuditorium;
    ...
    sscanf(str, "%d %s %s %s %d %d %d %f %f %d %d %d %f", 
        ...
        &coworkingCenter->distanceFromCityCenter, &auxMeetingRooms, &auxKitchen, &auxAuditorium,
        &coworkingCenter->percentOccupation );

    coworkingCenter->id = (tCoworkingCenterId)auxId;
    coworkingCenter->type = (tCoworkingType)auxType;
    coworkingCenter->hasMeetingRooms = (bool)auxMeetingRooms;
    if (auxKitchen != 0)
...
float coworkingCenterTableComputeAverageOccupation(tCoworkingCenterTable tabCoworkingCenter, char *city) {
	...
	float avgOcc;
	int totalSpaces, occupiedSpaces, i;
...
/*************** PR1 EX5A *****************/
	int i;
    tError retVal;
    tCoworkingCenterTable selectedCoworkingCenter;
    
	coworkingCenterTableInit(result);     
    
	coworkingCenterTableSelect(coworkingCenters,city,price,distance,points,&selectedCoworkingCenter);
    
	for(i=ZERO;i<selectedCoworkingCenter.nCoworkingCenters;i++) 
    {
		if (strcmp( coworker.birthCity, selectedCoworkingCenter.table[i].city ) != 0) {
			coworkingCenterTableAdd(result, selectedCoworkingCenter.table[i], &retVal);
		}
	}
/******************************************/
}
