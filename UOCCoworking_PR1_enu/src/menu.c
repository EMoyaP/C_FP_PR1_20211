...
/*************** PR1 EX5B *****************/
    ...
    tCoworkingCenterTable recommendedCoworkingCenter;
    
    coworkerTableSelectCoworkers(appData.coworkers, city, &selectedCoworkers);
    
	for(i=ZERO;i<selectedCoworkers.nCoworkers;i++) 
    {
        coworkerRecommendation(selectedCoworkers.table[i], appData.coworkingCenters,
                               city, price, distance, points, &recommendedCoworkingCenter );
        if (recommendedCoworkingCenter.nCoworkingCenters > ZERO) {
            printf("COWORKERS\n");
            printCoworker( selectedCoworkers.table[i], i+1);
        }
            
        for (j= ZERO; j < recommendedCoworkingCenter.nCoworkingCenters; j++) {
            printf("\t");
            printCoworkingCenter( recommendedCoworkingCenter.table[j], j+1 );            
        }
